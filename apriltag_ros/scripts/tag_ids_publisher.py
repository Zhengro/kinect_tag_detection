...
class TagDetection(object):
    """
    A combination of a subscriber that subscribes to the topic /tag_detections from apriltag_ros and a publisher that publishes only ids extracted from that topic.
    """
	...
	rospy.loginfo(data)  # /tag_detections
        ...
        rospy.loginfo(msg_data)  # detected_tag_ids
        print('\n')
